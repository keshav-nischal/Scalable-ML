...
- **Post:** "Unsupervised learning on messy real data — how I'd flag anomalies for [domain]."
...
- **Post:** "Transfer learning got me X% on N images/class — and here's exactly where it breaks."
...
- **Post:** "Classic ML still pays: forecasting [X] — XGBoost vs LSTM, and which one I'd actually ship."
...
- **Post:** "I built a production RAG API for [domain] — with the eval numbers most demos hide (faithfulness 0.62 → 0.89 after reranking)."
...
- **Post (signature):** "The part of LLM apps everyone skips: how I test and evaluate them so they don't rot in production — an ex-SDET's playbook." *(This post is your brand thesis. Make it excellent.)*
...
- **(a) Agentic workflow** — a **LangGraph** tool-using agent (e.g. a data-analyst that writes+runs Python, or a research/DB agent) with **self-correction, evals, and cost/latency guardrails**. Shows "systems that *act*," the fastest-growing category.
  ...
  - **Post:** "An AI agent that does real work — and the guardrails that stop it going rogue."
- **(b) MLOps service** — one of your models behind **Docker + CI + drift monitoring** (Evidently/Grafana) + **alerting** + a model registry.
  ...
  - **Post:** "Models decay. Here's the monitoring that tells you *before* your client does."
...
Char-level language model from scratch (Karpathy). On-ramp to autograd/backprop/embeddings. Writeup: "what backprop actually feels like by hand." *(Optional if time-pressed — nanoGPT subsumes it.)*

### R2 — nanoGPT / build-nanogpt  ·  ⭐ ~15–30 hrs (W13–W15)
Reproduce a **GPT-2-class** model (~300-line model + ~300-line training loop; scale down to your hardware — full 124M wants an 8×A100 node, but it runs small). **Highest-signal reproduction.** Writeup: "I reproduced GPT-2 from scratch — the 4 ideas that make a Transformer work."
