...
- The plan notes (`.claude/2026-study-plan/*.md`) and the interactive dashboard
  (`.claude/2026-study-plan/dashboard/dashboard.html`) are both AI-maintained and
  describe the **same** plan. Keep them in sync: **any change to the plan must also
  update the dashboard, and vice versa** — never edit one and leave the other stale.
- When changing week content, dates, blocks, projects, certs, checkpoints, posts, or
  numbering, apply the edit to both the relevant `.md` file(s) and the matching data
  in `dashboard.html`. After editing, verify the two still agree before committing.
